...
#' @export
int.binomial <- function (formula, data, max, link = "logit", ...) {
  ...
  if (any(lo > max | hi > max)) {
    stop("Limits cannot be greater than max")
  }
  lo <- ifelse(lo == 0, lo, lo - 1)
  X <- model.matrix(formula, dat.frame)
  k <- ncol(X)
  int.nLL <- function (params) {
    if (link == "logit") mu.hat <- plogis(X %*% params)
    else mu.hat <- pnorm(X %*% params)
    -sum(log(pbinom(hi, max, mu.hat) - pbinom(lo, max, mu.hat)))
  }
